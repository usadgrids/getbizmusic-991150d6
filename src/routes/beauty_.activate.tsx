...
export const Route = createFileRoute("/beauty_/activate")({
  ...
  head: () => ({
    meta: [
      { title: "Activate Your Beauty & Grooming Ad — Get Biz Music" },
      { name: "description", content: "Enter your activation code to review your beauty & grooming ad proof, confirm the details, and activate your GetBizMusic listing." },
      { property: "og:title", content: "Activate Your Beauty & Grooming Ad — Get Biz Music" },
      { property: "og:description", content: "Review your beauty & grooming ad proof, confirm your details, and activate your GetBizMusic listing." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: BeautyActivatePage,
});
      ...
      try {
        const res = await payInvoiceFn({
          data: {
            ...
            returnUrl: `${window.location.origin}/beauty/activate?code=${encodeURIComponent(search.code!)}&session_id={CHECKOUT_SESSION_ID}`,
          },
        });
    ...
    try {
      ...
      const res = await submitFn({
        data: {
          ...
          customerImagePath,
          environment: getStripeEnvironment(),
          returnUrl: `${window.location.origin}/beauty/activate?code=${encodeURIComponent(proof.code)}&session_id={CHECKOUT_SESSION_ID}`,
        },
      });
      ...
      <Shell>
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8 max-w-md mx-auto">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="font-serif text-2xl font-bold text-[#0F2A4A]">Activate your beauty & grooming ad</h1>
          </div>
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              void navigate({ to: "/beauty/activate", search: { code: codeInput.trim().toUpperCase() } });
    ...
    <div className="min-h-screen bg-[#f5f6f8]">
      ...
      <header className="bg-[#0F2A4A] text-white">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link to="/beauty" className="font-serif text-lg">Get Biz Music</Link>
          <span className="text-xs text-white/70">Beauty & Grooming Ad Activation</span>
        </div>
      </header>
